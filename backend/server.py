...
# Collections moved during the Emergent->Atlas data migration, with the
...
def _revive_dates(doc: dict) -> dict:
    """Convert ISO/str datetime values back to datetime for *_at keys so the
    target DB keeps proper date types (the sort pipelines rely on it)."""
...
@app.get("/api/debug/dump", include_in_schema=False)
async def debug_dump(collection: str, secret: str = "", skip: int = 0, limit: int = 50):
    """Read-only paginated dump of a collection (full documents, incl. base64).
    Used to export data from the Emergent-managed Mongo."""
    if secret != _DEBUG_ADMIN_SECRET:
        return {"error": "forbidden"}
    if collection not in _MIGRATE_COLLECTIONS:
    ...
    return {"collection": collection, "skip": skip, "limit": limit, "total": total, "count": len(docs), "docs": docs}


@app.post("/api/debug/load", include_in_schema=False)
async def debug_load(payload: dict, secret: str = ""):
    """Idempotent upsert of documents into a collection (by natural key).
    Used to import the exported data into the personal Atlas. Non-destructive:
    ...
    if collection not in _MIGRATE_COLLECTIONS:
        return {"error": f"collection not allowed: {collection}"}
    key = _MIGRATE_COLLECTIONS[collection]
    ...
    for d in docs:
        ...
        if kv is None:
            ...
            continue
        _revive_dates(d)
        await db[collection].update_one({key: kv}, {"$set": d}, upsert=True)
        upserted += 1
    return {"collection": collection, "upserted": upserted, "skipped": skipped}
